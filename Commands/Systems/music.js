  ...
  options: [
    {
      ...
      options: [
        {
          name: "query",
          description: "Indiquez un nom ou une URL pour la musique",
          type: "STRING",
          required: true,
        },
      ],
  ...
  async execute(interaction, client) {
    ...
    if (!VoiceChannel)
      return interaction.reply({
        content:
          "Vous devez être dans un canal vocal pour pouvoir utiliser les commandes musicales.",
        ephemeral: true,
      });
    ...
    if (
      ...
      return interaction.reply({
        content: `Je joue déjà de la musique dans <#${guild.me.voice.channelId}>.`,
        ephemeral: true,
      });
    ...
    try {
      switch (options.getSubcommand()) {
        ...
        case "volume": {
          ...
          return interaction.reply({
            content: `📶 Le volume a été réglé sur \`${Volume}%\``,
          });
        }
        case "settings": {
          ...
          switch (options.getString("options")) {
            case "skip":
              ...
              return interaction.reply({
                content: "⏭️ La musique a été skip.",
              });

            case "stop":
              await queue.stop(VoiceChannel);
              return interaction.reply({
                content: "⏭️ La musique a été stopper.",
              });

            case "pause":
              await queue.pause(VoiceChannel);
              return interaction.reply({
                content: "⏸️ La musique est en pause.",
              });
            ...
            case "AutoPlay":
              ...
              return interaction.reply({
                content: `🔃 Le mode de lecture automatique est réglé sur ${
                  Mode ? "On" : "Off"
                }`,
              });
            ...
            case "RelatedSong":
              ...
              return interaction.reply({
                content:
                  "🈁 Une chanson associée a été ajoutée à la file d'attente.",
              });

            case "RepeatMode":
              let Mode2 = await client.distube.setRepeatMode(queue);
              return interaction.reply({
                content: `🔃 Le mode de répétition est réglé sur ${
                  Mode2 == Mode2
                    ? Mode2 == 2
                      ? "File d'attente"
                      : "Chanson"
